...
void MainWindow::on_updateButton1_clicked()
{
    ...
    int stockId = ui->idEmploye->text().toInt(); // Assurez-vous que l'ID est correctement récupéré
    QString name = ui->type->text();
    int quantity = ui->quant->text().toInt();
    QString type = "Type";  // À ajuster selon votre interface
    ...
    QSqlQuery query;
    query.prepare("UPDATE STOCKS SET NAME_STOCK = ?, QUANTITY_STOCK = ?, TYPE_STOCK = ?, CONDITION_STOCK = ?, ENTRY_DATE_STOCK = ?, EMPLOYEE_ID = ? WHERE STOCK_ID = ?");
...
void MainWindow::on_sort_clicked()
{
    ...
    QSqlQuery query;
    query.prepare("SELECT STOCK_ID, NAME_STOCK, QUANTITY_STOCK, TYPE_STOCK, CONDITION_STOCK, ENTRY_DATE_STOCK, EMPLOYEE_ID "
...
void MainWindow::on_sauvegarder1_clicked() {
    ...
    if (stock.addStock()) {
        ...
        // Enregistrement dans l'historique
        QString details = QString("Produit : %1, Quantité : %2, Employé : %3 %4")
                              .arg(stock.getName())
